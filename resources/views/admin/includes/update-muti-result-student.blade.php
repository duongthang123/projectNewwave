    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">{{ __('message.Update') . ' ' . __('message.Score')}}</h4>
            ...
            {!! Form::open([
                ...
                <div class="modal-footer justify-content-between">
                    ...
                    <button type="button" 
                    onclick="event.preventDefault();updateMultiResultStudent(`{{ route('students.update-student-result', $student->id)}}`)" class="btn btn-primary">{{__('message.Update')}}</button>
                </div>
            {!! Form::close() !!}
        </div>
    </div>
